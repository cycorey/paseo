...
describe("TerminalManager - Command Execution", () => {
  ...
  describe("executeCommand", () => {
    ...
    it("should handle directory changes in command", async () => {
      const result = await manager.executeCommand("cd /tmp && pwd", process.env.HOME || "~", 5000);
    ...
    it("should launch interactive command (Python REPL)", async () => {
      const result = await manager.executeCommand("python3", process.env.HOME || "~", 5000);
    ...
    it("should handle command with working directory", async () => {
      const result = await manager.executeCommand("pwd", "/tmp", 5000);
  ...
  describe("listCommands", () => {
    it("should list all commands including dead ones", async () => {
      ...
      const result1 = await manager.executeCommand("echo 'test1'", process.env.HOME || "~", 5000);

      // Execute a command that stays running
      const result2 = await manager.executeCommand("python3", process.env.HOME || "~", 5000);
    ...
    it("should preserve command and working directory info for dead commands", async () => {
      ...
      const result1 = await manager.executeCommand(testCommand1, testDir1, 5000);

      const result2 = await manager.executeCommand(testCommand2, testDir2, 5000);
  ...
  describe("captureCommand", () => {
    ...
    it("should capture output from running command", async () => {
      const execResult = await manager.executeCommand("python3", process.env.HOME || "~", 5000);
  ...
  describe("sendTextToCommand", () => {
    ...
    it("should handle multiple sequential inputs to REPL", async () => {
      ...
      const output = await manager.sendTextToCommand(execResult.commandId, "print(x + y)", true, {
        lines: 50,
        maxWait: 2000,
      });
  ...
  describe("sendKeysToCommand", () => {
    it("should send Ctrl-C to interrupt running command", async () => {
      ...
      // Send Ctrl-C
      await manager.sendKeysToCommand(execResult.commandId, "C-c", 1, { lines: 50, maxWait: 2000 });
    ...
    it("should send Enter key to REPL", async () => {
      ...
      const output = await manager.sendKeysToCommand(execResult.commandId, "Enter", 1, {
        lines: 50,
        maxWait: 2000,
      });
  ...
  describe("killCommand", () => {
    it("should kill running command", async () => {
      const execResult = await manager.executeCommand("sleep 100", process.env.HOME || "~", 1000);
  ...
  describe("Complex workflows", () => {
    it("should handle Node.js REPL workflow", async () => {
      // Launch Node REPL
      const execResult = await manager.executeCommand("node", process.env.HOME || "~", 5000);
  ...
  describe("Edge cases", () => {
    ...
    it("should handle command with tilde in directory", async () => {
      const execResult = await manager.executeCommand("pwd", "~/", 5000);
